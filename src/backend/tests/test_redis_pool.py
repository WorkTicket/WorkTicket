...
async def test_get_sync_redis_returns_client():
    ...
    with (
        patch("app.sync_redis_pool.SyncRedisPool._connect", return_value=True),
        patch("app.sync_redis_pool.SyncRedisPool._pool", new_callable=PropertyMock, return_value=mock_pool),
    ):
...
async def test_get_sync_redis_returns_none_when_unavailable():
    ...
    with patch("app.sync_redis_pool.SyncRedisPool._connect", return_value=False):
        client = get_sync_redis()
...
async def test_pool_reconnects_after_failure():
    ...
    with patch("app.sync_redis_pool.SyncRedisPool._connect", side_effect=connect_side_effect):
...
async def test_pool_stats_report_metrics():
    ...
    type(mock_pool).connection_pool = PropertyMock(return_value=mock_conn_pool)
