...
async def test_webhook_rate_limiter_redis_fallback_uses_local():
    from app.billing.router import _check_webhook_rate, _webhook_rate

    _webhook_rate.clear()

    with patch("app.billing.router._get_redis", AsyncMock(return_value=None)):
...
async def test_webhook_rate_limiter_redis_exception_fallback():
    from app.billing.router import _check_webhook_rate, _webhook_rate, _webhook_rate_limit
    ...
    with patch("app.billing.router._get_redis", return_value=mock_redis):
...
async def test_webhook_rate_limiter_redis_blocks_excess():
    ...
    with patch("app.billing.router._get_redis", return_value=mock_redis), pytest.raises(HTTPException) as exc_info:
...
async def test_webhook_rate_limiter_local_cleanup_expired():
    from app.billing.router import _check_webhook_rate, _webhook_rate, _webhook_window
