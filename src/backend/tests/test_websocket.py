...
def _auto_mocks(mock_user):
    with (
        ...
        incr.return_value = (True, 4)
        # Make DB poll fail silently (caught by except Exception)
        mock_db.side_effect = RuntimeError("Test DB unavailable")
        yield {
            "verify": verify,
            "incr": incr,
            "decr": decr,
...
def test_websocket_missing_token():
    ...
    with pytest.raises(Exception), client.websocket_connect(f"{WS_PATH}/{uuid.uuid4()}"):
        pass
    # Connection should be refused before accept
...
def test_websocket_rate_limit_closes_connection(_auto_mocks):
    """H1/R3: 61 data messages trigger rate limit; connection closes and cleanup runs."""
    mocks = _auto_mocks
    client = TestClient(app)
    job_id = uuid.uuid4()
    try:
        with client.websocket_connect(f"{WS_PATH}/{job_id}?token={MOCK_TOKEN}") as ws:
            for _i in range(61):
                try:
                    ws.send_text("data")
                except Exception:
    ...
    # Cleanup should have been called once (increment then decrement)
    mocks["incr"].assert_awaited_once_with(MOCK_USER_ID)
    mocks["decr"].assert_awaited_once_with(MOCK_USER_ID)


def test_websocket_cleanup_on_normal_disconnect(_auto_mocks):
...
def test_websocket_rate_limit_connection_refused(_auto_mocks):
    """H1: Connection rate limit (initial phase) closes with 1008 before accept."""
    mocks = _auto_mocks
    mocks["incr"].return_value = (False, 0)

    client = TestClient(app)
    with pytest.raises(Exception), client.websocket_connect(f"{WS_PATH}/{uuid.uuid4()}?token={MOCK_TOKEN}"):
        pass
...
def test_websocket_rate_limit_sends_error_before_close(_auto_mocks):
...
def test_websocket_connection_limit_exceeded_after_accept(_auto_mocks):
...
def test_websocket_token_from_header():
...
def test_websocket_redis_unavailable_fallback(_auto_mocks):
    """H2: When Redis is unavailable, local fallback handles tracking."""
    mocks = _auto_mocks
    mocks["incr"].return_value = (True, 4)
...
def test_websocket_decrement_called_on_poll_exception(_auto_mocks):
...
def test_websocket_timestamps_cleared_on_disconnect(_auto_mocks):
...
def test_websocket_ping_pong_does_not_accumulate(_auto_mocks):
...
def test_websocket_ping_does_not_count_toward_rate_limit(_auto_mocks):
    ...
    try:
        with client.websocket_connect(f"{WS_PATH}/{job_id}?token={MOCK_TOKEN}") as ws:
            # Send 100 pings — these must not count
            for _i in range(100):
                ws.send_text("ping")
                resp = ws.receive_json()
                assert resp.get("type") == "pong", f"Expected pong, got {resp}"

            # Send 61 data messages — should NOT trigger rate limit since pings were excluded
            for _i in range(61):
                ws.send_text("data")
...
def test_websocket_connection_limit_enforced(_auto_mocks):
    ...
    client = TestClient(app)
    job_id = uuid.uuid4()
    with pytest.raises(Exception), client.websocket_connect(f"{WS_PATH}/{job_id}?token={MOCK_TOKEN}"):
        pass

    mocks["incr"].assert_awaited_once_with(MOCK_USER_ID)
    # Decrement should NOT be called since connection was never fully opened
    mocks["decr"].assert_not_called()


def test_websocket_redis_tracking_called(_auto_mocks):
