...
async def test_trace_record_and_retrieve(client: AsyncClient):
    ...
    await record_trace(trace_id, "api_receive", "started", job_id=job_id, company_id=company_id)
    ...
    response = await client.get(f"/analytics/traces/{trace_id}")
    assert response.status_code == 200
    data = response.json()
    assert data["trace_id"] == trace_id
    assert len(data["steps"]) == 3
    assert data["status"] == "completed"
    assert data["total_duration_ms"] == 1205.0


@pytest.mark.asyncio
async def test_trace_not_found(client: AsyncClient):
    fake_id = uuid.uuid4()
    response = await client.get(f"/analytics/traces/{fake_id}")
...
async def test_trace_metrics_endpoint(client: AsyncClient):
    ...
    response = await client.get("/analytics/traces/metrics/summary?minutes=1440")
    assert response.status_code == 200
    data = response.json()
